...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  useEffect(() => {
    if (
      getValues("gender") == "regionales" &&
      getValues("type") != "espadrilles"
    ) {
      setIsHidden(true);
    } else {
      setIsHidden(false);
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            <Box display="flex" justifyContent="space-around">
              ...
              {isHidden ? null : (
                <FormGroup>
                  <FormLabel sx={{mb:2}}>STOCK</FormLabel>
                  {getValues("sizes").map((size) => (
                    <Box key={size} sx={{m:1}}>
                      <TextField
                        type="number"
                        label={size}
                        onChange={(e) =>
                          onChangeTalla(size, parseInt(e.target.value))
                        }
                      />
                    </Box>
                  ))}
                  <Box display="flex" justifyContent="center">
                    ...
                    onClick={()=> setValue('talles', tallas_)}
                    >
                      Enviar
                    </Button>
                  </Box>
